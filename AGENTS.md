...
`make sbom` reproduces the SBOM published with a release. It owns the scan target and the three `SYFT_*` overrides that make the inventory correct (#136, #139), and runs syft from a digest-pinned stage in the `Dockerfile` — which is what makes the version both explicit and Dependabot-tracked, since Dependabot does not parse action inputs. `release.yml` calls this target rather than invoking syft itself, so the published artifact is reproducible from a clean checkout.
