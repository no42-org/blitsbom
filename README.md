...
curl -fLO https://github.com/no42-org/blitsbom/releases/download/v0.8.2/blitsbom-0.8.2.zip
curl -fLO https://github.com/no42-org/blitsbom/releases/download/v0.8.2/blitsbom-0.8.2.sha512

# Verify integrity (exits non-zero if the bundle was tampered with)
sha512sum -c blitsbom-0.8.2.sha512
```
...
# A specific version
docker run --rm -p 8080:3000 ghcr.io/no42-org/blitsbom:0.8.2
...
```yaml
- uses: no42-org/blitsbom@v0.8.2
  id: sbom-report
  with:
    sbom: bom.json          # path to your CycloneDX or SPDX SBOM
    # image defaults to :report-0.8.2, matching the action tag
...
|---|---|
| `@v0.8.2` — a release tag | `:report-0.8.2` |
...
docker run --rm -v "$PWD:/work" ghcr.io/no42-org/blitsbom:report-0.8.2 \
